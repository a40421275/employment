...
public interface JobRepository extends JpaRepository<Job, Long>, JpaSpecificationExecutor<Job>, JobRepositoryCustom {
     ...
     * 根据ID查询岗位与企业信息（投影查询）
     */
    @Query("SELECT new com.shera.framework.employment.employment.modules.job.dto.JobWithCompanyDTO(" +
    ...
    /**
     * 增加岗位浏览次数（使用原生SQL避免锁表）
     */
    @Modifying
    @Query(value = "UPDATE job SET view_count = view_count + 1 WHERE id = :id", nativeQuery = true)
